 ...
 *
 */
(function(i,x){var n=x.navigator;i.support.geolocation=n.geolocation;i.whereNow=function(b,d,a,f){return j.instance(b,d,a,f)};var l=null,m=null,h=function(b){return b*(Math.PI/180)},q=function(b,d){var a=h(b.lat-d.latitude),f=h(b.lng-d.longitude);a=Math.sin(a/2)*Math.sin(a/2)+Math.cos(h(b.lat))*Math.cos(h(d.latitude))*Math.sin(h(f)/2)*Math.sin(h(f)/2);return 6371*2*Math.atan(Math.sqrt(a),Math.sqrt(1-a))},j=function(b,d){var a=this,f=false,e=i.extend({},this.defaults,d),r=new Date;this.locations=b;
